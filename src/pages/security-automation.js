...
const SecurityAutomation = () => {
  const data = useStaticQuery(graphql`
    query securityautomationsecurity {
      contentfulHero(pageId: { eq: "security-automation" }) {
        button1
        description1
        mainHeading
        image1 {
      ...
      allContentfulPostHero(filter: { pageId: { eq: "security-automation" } }) {
        edges {
          node {
            mainHeading
            description
      ...
      contentfulSection1(itemId: { eq: "security-automation-section1" }) {
        description
        mainHeading
        list {
          heading
      ...
      allContentfulSwithContent(
        filter: { pageId: { eq: "security-automation" } }
      ) {
        edges {
          node {
            description
      ...
      contentfulQuotes(pageId: { eq: "security-automation" }) {
        image {
          url
        }
        description
      ...
      allContentfulItemCard(filter: { pageId: { eq: "security-automation" } }) {
        edges {
          node {
            mainHeading
            image1 {
...
`}
      />
      <Overview list={OverviewList} title={`security-automation Security`} />
